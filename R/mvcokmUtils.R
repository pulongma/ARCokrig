...
#'   autogressive cokriging models for multivariate outputs
#'  
...
mvcokm <- function(formula=list(~1,~1), output, input, param, cov.model="matern_5_2",
	prior=list(), opt=list(), NestDesign=TRUE, tuning=list(), info=list()){

## check the arguments 
  .check.arg.mvcokm(formula=formula, output=output, input=input, param=param,
...
#####################################################################
#####################################################################
.check.arg.mvcokm <- function(formula, output, input, param, prior, opt, 
